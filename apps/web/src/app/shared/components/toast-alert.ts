...
@Component({
  ...
  template: `
    @if (copyStore.isCopied()) {
        ...
        <div
            class="fixed bottom-4 left-1/2 z-50 w-max max-w-sm -translate-x-1/2 rounded-lg border border-red-500/30 bg-red-950/95 px-4 py-3 text-red-100 shadow-lg backdrop-blur-sm"
            role="status"
            aria-live="polite"
        >
